...
const CACHE_VERSION = 2; // bump when rule format changes — forces re-download
...
export class BlockerEngine {
    ...
    #parseNetworkRule(line) {
        ...
        // Tokenize pattern: escape literals FIRST, then convert AdBlock tokens
        // This avoids breaking regex metacharacters generated by token conversion
        let regex;
        try {
            let regexStr = "";
            let i = 0;
            ...
            while (i < pattern.length) {
                if (pattern[i] === "|" && pattern[i + 1] === "|") {
                    ...
                    // Literal character — escape if it's a regex metacharacter
                    const ch = pattern[i];
                    if (".+?{}()[]\\/$|".includes(ch)) {
                        regexStr += "\\" + ch;
                    } else {
    ...
    async #loadCached() {
        try {
            ...
            // Invalidate cache if format version changed
            if (data.version !== CACHE_VERSION) {
                console.log("[Blocker] Cache version mismatch — will re-download lists");
                return;
            }
